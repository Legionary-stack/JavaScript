...
function CaesarEncode(s,shift, alphabet) {
    ...
    const len = s.length
    const negative = Math.floor(Math.abs(shift) / letters_count) + 1
    let new_s = ""
    const reg = letters_count > 30 ? /^[А-яёЁ]$/ : /^[A-z]$/
    for(i = 0; i < len; i++) {
        if (reg.test(s[i])) {
            const temp = alphabet[letters_count] > s[i] ? 0 : letters_count
            const index = alphabet.indexOf(s[i])
            new_s += alphabet[(index + shift + letters_count * negative) % letters_count + temp]
        }
        else
            new_s += s[i]
    }  
